 ...
 * Each archive entry contains the post's title, slug, traveled date (or publication date if traveled is not set), and tags.
...
export async function GetArchives() {
  ...
  for (const post of allBlogPosts) {
    // Only include posts with a traveled date in the timeline
    if (!post.data.traveled) continue;

    const date = new Date(post.data.traveled);
